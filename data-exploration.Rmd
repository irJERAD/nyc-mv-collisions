...
Our Dataset structure revealed the variables and their classes `sapply(names(dt), function(x) paste0(x, ' is class: ', class(dt[[x]])))` = `r sapply(names(dt), function(i) {paste0(i,' is class: ',class(dt[,][[i]]))})`

`sapply(names(dt), function(x) paste0('<br>','<b>',x,'</b>','<em>',' is class: ','</em>','<u>',class(dt[[x]]),'</u>'))`
...
gg <- gg + labs(x = "Hour of the Day 00:00 _\\/_ 23:00 ",
                y = "Day of the Week",
                title = "Folding Time")
